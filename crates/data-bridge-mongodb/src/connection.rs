...
impl Default for PoolConfig {
    fn default() -> Self {
        Self {
            ...
            app_name: Some("data-bridge".to_string()),
        }
    }
}
...
impl Connection {
    ...
    pub async fn with_config(connection_string: &str, config: PoolConfig) -> Result<Self> {
        ...
        let database = client.default_database().ok_or_else(|| {
            data_bridge_common::DataBridgeError::Connection(
    ...
    pub async fn ping(&self) -> Result<bool> {
        match self.database.run_command(doc! { "ping": 1 }).await {
            Ok(_) => Ok(true),
            Err(e) => Err(data_bridge_common::DataBridgeError::Connection(format!(
                "Ping failed: {}",
                e
            ))),
        }
